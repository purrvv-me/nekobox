...
function Row({
  ...
  const cls = `group relative flex h-8 items-center gap-2.5 rounded-md px-2.5 transition-colors ${
    active ? "bg-black/[0.06]" : "hover:bg-black/[0.045]"
  }`;
  const bar = (
    <span
...
export function NavPane({
  ...
  return (
    <div className="w11-scroll w-[226px] flex-shrink-0 overflow-y-auto border-r border-line bg-nav px-2 py-2.5">
      ...
      {folders && (
        <>
          <div className="mx-2 my-2 h-px bg-[#e6e9ef]" />
          <p className="mx-2 mb-1.5 mt-1 text-[11px] font-semibold text-sub">This vault</p>
          {folders.length === 0 ? (
            <p className="px-2.5 py-1 text-[12px] text-faint">No folders yet</p>
          ) : (
            folders.map((f) => {
